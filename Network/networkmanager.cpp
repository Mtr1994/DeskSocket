...
{
    // 信号管理
    connect(AppSignal::getInstance(), &AppSignal::sgl_create_network_object, this, &NetworkManager::slot_create_network_object);
}

void NetworkManager::slot_create_network_object(int type, const QString &address, uint16_t port)
{
    ...
    connect(AppSignal::getInstance(), &AppSignal::sgl_recreate_network_object, object, &NetworkObject::slot_recreate_network_object);
